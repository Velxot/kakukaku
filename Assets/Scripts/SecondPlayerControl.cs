...
public class SecondPlayerControlScript : MonoBehaviour
{
    [SerializeField]
    ...
    void HandleInput()
    {
        ...
        if (Input.GetKeyDown(KeyCode.RightArrow) && posNum < WIDTH - 1)
        {
            posNum++;
            UpdateBlockPosition();
        }
    ...
    void SpawnBlock()
    {
        GameObject newBlock = Instantiate(blockPrefab, new Vector3(1f+posNum * 0.5f, -2f, 0), Quaternion.identity);
    ...
    void UpdateBlockPosition()
    {
        if (blockObjects.Count > 0)
        {
            ...
            {
                currentBlock.transform.position = new Vector3(1f+posNum * 0.5f, -2f, 0);
            }
        }
    }
    ...
    void UpdateFieldDisplay()
    {
        ...
        for (int y = 0; y < HEIGHT; y++)
        {
            for (int x = 0; x < WIDTH; x++)
            {
                if (field[y, x] != 0)
                {
                    GameObject newBlock = Instantiate(blockPrefab, new Vector3(1f+x * 0.5f, -y * 0.5f + 4, 0), Quaternion.identity);
                    SpriteRenderer sr = newBlock.GetComponent<SpriteRenderer>();
